...
As you have seen in the previous examples, and you will corroborate in the remaining
...
in those cases you may have to recur to invoking the \code{GetInverse()} method that
...
We now present the multi-resolutionary capabilities of the framework by
way of an example.
...
to do last adjustments in full resolution.
Note that by this approach we can save large amounts of time when we aim to run
a deformable registration process on a large dataset like a high-resolution 3D
image, since we need to run much less iterations at the full resolution level.
