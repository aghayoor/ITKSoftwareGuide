...
\item ANTS neighborhood correlaiton metric \\ \doxygen{ANTSNeighborhoodCorrelationImageToImageMetricv4}
\end{itemize}

Also, in the case that you are still interested to use the legacy ITK registration framework,
...
and is set using method \code{SetMetricSamplingPercentage()} directly from
...
via \code{SetNumberOfHistogramBins()} method.
