...
of registration, \doxygen{ImageToImageMetric} classes provides this functionality.
...
The \doxygen{SingleValuedNonLinearOptimizer} is the base class for the first
type of optimizers while the \doxygen{MultipleValuedNonLinearOptimizer} is the
...
(\doxygen{GradientDescentOptimizer}).
...
(\doxygen{QuaternionRigidTransformGradientDescent}).
...
in~\cite{Byrd1995,Zhu1997} (\doxygen{LBFGSBOptimizer}).
...
(\doxygen{OnePlusOneEvolutionaryOptimizer.}). Details on this optimizer can be
...
in~\cite{Press1992}.  (\doxygen{PowellOptimizer}).
...
(\doxygen{VersorRigid3DTransformOptimizer}).
